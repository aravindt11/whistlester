...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        self.barTintColor=[UIColor peterRiverColor ];
        //self.titleTextAttributes = @{NSForegroundColorAttributeName: [UIColor cloudsColor]};
         self.titleTextAttributes = @{NSFontAttributeName: [UIFont boldFlatFontOfSize:15.0f],NSForegroundColorAttributeName: [UIColor whiteColor]};
    
        
        /*NSArray *items= self.items;
        for (UIView *subView in items) {
            if ([subView isKindOfClass:[UINavigationItem class]]) {
            
                UINavigationItem *buttonItem=(UINavigationItem *)subView;
                buttonItem.title
                [buttonItem setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:
                                                    [UIFont boldFlatFontOfSize:15.0f], NSFontAttributeName,
